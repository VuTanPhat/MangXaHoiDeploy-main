  ...
  const handleDeleteStory = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setViewStory(null);
        if (onStoryDeleted) onStoryDeleted(viewStory._id);
      } else {
    ...
    >
      ...
      {viewStory.user?._id === currentUser?._id && (
        <button
          ...
          disabled={deleting}
          className="absolute top-4 right-16 text-white focus:outline-none p-2 rounded-full bg-red-500/80 hover:bg-red-600 transition disabled:opacity-50"
        >
